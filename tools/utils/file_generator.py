...
def read_config(config):
    ...
    chunk_size = config.get('CHUNK_SIZE', 32)  # Default to 32
    return block_m, block_n, block_k, group_m, num_sms, num_warps, num_stages, waves_per_eu, mfma_instr_size, kpack, chunk_size
...
def gen_configStr(config):
    ...
    configStr = f"BM{block_m}_BN{block_n}_BK{block_k}_GM{group_m}_nSM{num_sms}_nW{num_warps}_nS{num_stages}_EU{waves_per_eu}_kP{kpack}_mfma{mfmaInstrSize}_CH{chunk_size}"
...
from tritonblas.internal.pid_transforms import chiplet_transform_chunked
"""
    f_kernel.write(import_str)

    # Convert module name to file path
    # e.g., "tritonblas.internal.streamk_matmul" -> "../../include/tritonblas/internal/streamk_matmul.py"
    # From utils/ directory: ../ → tools/, ../../ → tritonBLAS_new/
    if '.' in module_name:
    ...
    for config in configs:
        ...
        matmul_kernel_config = matmul_kernel_config.replace("from .pid_transforms import chiplet_transform_chunked", "")
...
def gen_kernel_and_configStr_from_config(config, EVEN_K, dtype_a, dtype_b, dtype_c, dtype_p, dtype_lock, bias_size,
                                         warmup, kernel_name, kernel_type='streamk'):
    block_m, block_n, block_k, group_m, num_sms, num_warps, num_stages, waves_per_eu, mfmaInstrSize, kpack, chunk_size = read_config(
        config)
...
def generate_compile_driver(M, N, K, col_a, col_b, dtype_a, dtype_b, dtype_c, dtype_p, dtype_lock, init_type, configs,
                            rotating_buffer_size, bias_size, kernel_name, kernel_type='streamk'):
    """
...
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'include'))
# from triton_gemm_bench import gen_rotating_tensors  # Not needed for compilation
# from tritonblas.internal.pid_transforms import chiplet_transform_chunked  # Not needed for compilation
...
"""
    ...
    # Set multiprocessing to spawn mode to avoid CUDA reinitialization issues
...
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'include'))
from triton_gemm_bench import gen_rotating_tensors
from tritonblas.internal.pid_transforms import chiplet_transform_chunked
"""
    if icache_flush:
        import_str += """
...
"""
    ...
    # write test_gemm
    # pre string
    # Allocate shared buffers once per file for StreamK
    if kernel_type == 'streamk':
        # Get max num_sms, block_m, block_n from configs for this file
        max_num_sms = max([c.get('NUM_SMS') for c in configs])
        max_block_m = max([c.get('BLOCK_SIZE_M') for c in configs])
        max_block_n = max([c.get('BLOCK_SIZE_N') for c in configs])
...
"""
    ...
    if '{configStr}' not in failed_configs:
        ...
        for i in range({runs}):
            ...
            # StreamK kernel - reuse shared P, locks
            matmul_call_str = f"""
    if '{configStr}' not in failed_configs:
        rotating_num = tensors['rotating_num']
        for i in range({runs}):
